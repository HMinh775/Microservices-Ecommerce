...
export default function Checkout() {
  ...
  // Nhận cart từ trang Giỏ hàng HOẶC reorderItems từ trang Lịch sử đơn hàng
  const reorderItems = location.state?.reorderItems || [];
  const cart = location.state?.cart || reorderItems;
  const subtotal = cart.reduce((sum, item) => sum + (item.variantInfo?.price || 0) * item.quantity, 0);
  const total = location.state?.total || (subtotal > 500000 ? subtotal : subtotal + 30000);
  ...
  useEffect(() => {
    ...
    // Chỉ redirect về cart nếu thực sự không có items
    if (cart.length === 0) {
      navigate('/cart');
    }
  }, []);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // 2. Chỉ xóa giỏ hàng ngay lập tức nếu là COD
